...
def test_run():
    ...
    end_date = datetime.date(2016,12,31)    #date.today() datetime.date(2013,12,31)
    ...
    small30_10D = rf.get_rolling_small(day_10_returns.iloc[-200-n:,:],200,60)
...
#    writer = cmn.to_file(large30_10D.sort_index(ascending=False, inplace=False),"B3_large30_10D",csv,str_trade_date)
    ...
    small30_15D = rf.get_rolling_small(day_15_returns.iloc[-200-n:,:],200,60)
...
#    writer = cmn.to_file(large30_15D.sort_index(ascending=False, inplace=False),"B5_large30_15D",csv,str_trade_date)
    ...
    small30_5D = rf.get_rolling_small(day_5_returns.iloc[-200-n:,:],200,60)
...
#    writer = cmn.to_file(large30_5D.sort_index(ascending=False, inplace=False),"B7_large30_5D",csv,str_trade_date)
    ...
    ra_15DHL = rf.get_rolling_nDHL(df_Stock.iloc[-215-n:,:],window=15,n=15)
    small30_15DHL = rf.get_rolling_small(ra_15DHL.iloc[-200-n:,:],200,60)
...
#    writer = cmn.to_file(large30_15DHL.sort_index(ascending=False, inplace=False),"B10_large30_15DHL",csv,str_trade_date)
    ...
    ra_10DHL = rf.get_rolling_nDHL(df_Stock.iloc[-210-n:,:],window=10,n=10)
    small30_10DHL = rf.get_rolling_small(ra_10DHL.iloc[-200-n:,:],200,60)
...
#    writer = cmn.to_file(large30_10DHL.sort_index(ascending=False, inplace=False),"B13_large30_10DHL",csv,str_trade_date)
    ...
    ra_5DHL = rf.get_rolling_nDHL(df_Stock.iloc[-205-n:,:],window=5,n=5)
    small30_5DHL = rf.get_rolling_small(ra_5DHL.iloc[-200-n:,:],200,60)
#    large30_5DHL = rf.get_rolling_small(ra_5DHL.iloc[-200-n:,:],1200,840)
...
#    writer = cmn.to_file(large30_5DHL.sort_index(ascending=False, inplace=False),"B16_large30_5DHL",csv,str_trade_date)
    ...
    large_15D = rf.get_rolling_small(df_Stock.shift(1).iloc[-15-n:,:],15,15)
    large_15D.dropna(how='all',inplace=True)
    small_15D = rf.get_rolling_small(df_Stock.shift(1).iloc[-15-n:,:],15,1)
    ...
    array_cntr = np.where(Cntr_Breakout.isnull() ,np.nan,
                      np.where(Cntr_Breakout == 'B',((1.75*ra_15DHL.iloc[-1-n:,:]+1)*df_Stock.iloc[-1-n:,:]),
                      np.where(Cntr_Breakout == 'S',((1.5*ra_15DHL.iloc[-1-n:,:]+1)*df_Stock.iloc[-1-n:,:]),np.nan)))
    indx = Cntr_Breakout.index.values
    Breakout_Profit = pd.DataFrame(array_cntr,index=indx ,columns=list_nse50)
    writer = cmn.to_file(Breakout_Profit.sort_index(ascending=False, inplace=False),"B20_Breakout_Profit",csv,str_trade_date)

    array_cntr = np.where(Cntr_Breakout.isnull() ,np.nan,
                          np.where((Cntr_Breakout == 'B') | (Cntr_Breakout == 'S') ,
                               ((1.1*ra_5DHL.iloc[-1-n:,:]+1)*df_Stock.iloc[-1-n:,:]),np.nan))
    indx = Cntr_Breakout.index.values
    Breakout_Loss = pd.DataFrame(array_cntr,index=indx ,columns=list_nse50)
    writer = cmn.to_file(Breakout_Loss.sort_index(ascending=False, inplace=False),"B21_Breakout_Loss",csv,str_trade_date)
