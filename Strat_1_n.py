...
def test_run():
    ...
    end_date = datetime.date(2018,9,5)    #date.today() datetime.date(2013,12,31)
    ...
    #writer = cmn.to_file(df_Stock.iloc[-n:,:].sort_index(ascending=False, inplace=False) ,"1_StockPrice",csv,str_trade_date)
    #writer = cmn.to_file(df_Stock,"StockPrice2",excel,str_trade_date,writer)  
    
    rm_200 = rf.get_rolling_mean(df_Stock.iloc[-200-n:,:], window=200)
    rm_200.dropna(how='all',inplace=True) 
    #writer = cmn.to_file(rm_200.sort_index(ascending=False, inplace=False),"2_RollMean200",csv,str_trade_date)
    
    rm_50 = rf.get_rolling_mean(df_Stock.iloc[-50-n:,:], window=50)
    rm_50.dropna(how='all',inplace=True) 
    ...
    day_1_returns.dropna(how='all',inplace=True)  
    #writer = cmn.to_file(day_1_returns.iloc[-1:,:].sort_index(ascending=False, inplace=False),"4_OneDayRet",csv,str_trade_date)
    ...
    rm_abs1day_50 = rf.get_rolling_mean(abs_day_1_returns.iloc[-50-n:,:], window=50)
    rm_50.dropna(how='all',inplace=True) 
    ...
    #writer = cmn.to_file(day_10_returns.iloc[-10-n:,:].sort_index(ascending=False, inplace=False),"7_10DayRet",csv,str_trade_date)
   
    
    rd_10DRet = rf.get_rolling_std(day_10_returns.iloc[-1200-n:,:], window=1200)
    rd_10DRet.dropna(how='all',inplace=True) 
    #writer = cmn.to_file(rd_10DRet.sort_index(ascending=False, inplace=False),"8_10DayRetStdDev",csv,str_trade_date)
     
    ra_10plarge = rf.get_rolling_nAvg(df_Stock.iloc[-1200-n:,:],1200,120,120,40)
    ra_10plarge.dropna(how='all',inplace=True) 
    #writer = cmn.to_file(ra_10plarge.sort_index(ascending=False, inplace=False),"9_10PCLarge",csv,str_trade_date)
    
    ra_10psmall = rf.get_rolling_nAvg(df_Stock.iloc[-1200-n:,:],1200,1080,1080,1160)
    ra_10psmall.dropna(how='all',inplace=True)
    #writer = cmn.to_file(ra_10psmall.sort_index(ascending=False, inplace=False),"10_10PCSmall",csv,str_trade_date)
 
    ra_3plarge = rf.get_rolling_nAvg(df_Stock.iloc[-1200-n:,:],1200,40,40,13)
    ra_3plarge.dropna(how='all',inplace=True)
    #writer = cmn.to_file(ra_3plarge.sort_index(ascending=False, inplace=False),"11_3PCLarge",csv,str_trade_date)
 
    ra_3psmall = rf.get_rolling_nAvg(df_Stock.iloc[-1200-n:,:],1200,1140,1160,1147)
    ra_3psmall.dropna(how='all',inplace=True)
    #writer = cmn.to_file(ra_3psmall.sort_index(ascending=False, inplace=False),"12_3PCSmall",csv,str_trade_date)
 
    ra_15plarge = rf.get_rolling_nAvg(df_Stock.iloc[-1200-n:,:],1200,120,180,180)
    ra_15plarge.dropna(how='all',inplace=True)
    #writer = cmn.to_file(ra_15plarge.sort_index(ascending=False, inplace=False),"13_15PCLarge",csv,str_trade_date)
 
    ra_15psmall = rf.get_rolling_nAvg(df_Stock.iloc[-1200-n:,:],1200,1080,1020,1120)
    ra_15psmall.dropna(how='all',inplace=True)
    ...
    array_cntr = np.where(ra_3psmall.isnull() ,np.nan,
                      np.where(day_10_returns.iloc[-1-n]*100 <= ra_3psmall/100,3,
                      np.where(day_10_returns.iloc[-1-n]*100 <= ra_10psmall/100,2,
                      np.where(day_10_returns.iloc[-1-n]*100 <= ra_15psmall/100,1,0))))
    ...
    array_cntr = np.where(ra_3plarge.isnull() ,np.nan,
                      np.where(day_10_returns.iloc[-1-n]*100 > ra_3plarge/100,3,
                      np.where(day_10_returns.iloc[-1-n]*100 > ra_10plarge/100,2,
                      np.where(day_10_returns.iloc[-1-n]*100 > ra_15plarge/100,1,0))))
    ...
    rRSI_13 = rf.get_Rolling_RSI(df_Stock.iloc[-14-n:,:],13)
    rRSI_13.dropna(how='all',inplace=True)
    ...
    vwap1 = rf.get_rolling_VMAP(df_Stock.iloc[-200-n:,:],200,1)
    vwap1.dropna(how='all',inplace=True)
    #writer = cmn.to_file(vwap1.sort_index(ascending=False, inplace=False) ,"20_VWAP1",csv,str_trade_date)
   
    vwap2 = rf.get_rolling_VMAP(df_Stock.iloc[-200-n:,:],200,2)
    vwap2.dropna(how='all',inplace=True)
    #writer = cmn.to_file(vwap2.sort_index(ascending=False, inplace=False) ,"21_VWAP2",csv,str_trade_date)
 
    vwap3 = rf.get_rolling_VMAP(df_Stock.iloc[-200-n:,:],200,3)
    vwap3.dropna(how='all',inplace=True)
    #writer = cmn.to_file(vwap2.sort_index(ascending=False, inplace=False) ,"22_VWAP3",csv,str_trade_date)
    
    v1_ratio = np.where(vwap1.isnull() ,np.nan,(df_Stock.iloc[-1-n:,:] - vwap1)/df_Stock.iloc[-1-n:,:])
    v2_ratio = np.where(vwap1.isnull() ,np.nan,(df_Stock.iloc[-1-n:,:] - vwap2)/df_Stock.iloc[-1-n:,:])
    v3_ratio = np.where(vwap1.isnull() ,np.nan,(df_Stock.iloc[-1-n:,:] - vwap3)/df_Stock.iloc[-1-n:,:])
    ...
    df_dma50 = np.where(rm_50.isnull() ,np.nan,(df_Stock.iloc[-1-n:,:] - rm_50)/df_Stock.iloc[-1-n:,:])
    df_dma200 = np.where(rm_200.isnull() ,np.nan,(df_Stock.iloc[-1-n:,:] - rm_200)/df_Stock.iloc[-1-n:,:])
    ...
    rs_SLBuy = rf.get_rolling_small(df_Stock.iloc[-1100-n:,:],1100,1)
    rs_SLBuy.dropna(how='all',inplace=True)
    ...
    array_cntr = np.where(rs_SLBuy.isnull() ,np.nan,
                          np.where((((df_Stock.iloc[-1-n:,:] - rs_SLBuy).abs())/df_Stock.iloc[-1-n:,:])<0.025,2,0))
    ...
    rs_SLSell = rf.get_rolling_small(df_Stock.iloc[-1100-n:,:],1100,1100)
    rs_SLSell.dropna(how='all',inplace=True)
    ...
    array_cntr = np.where(rs_SLSell.isnull() ,np.nan,
                          np.where((((df_Stock.iloc[-1-n:,:] - rs_SLSell).abs())/df_Stock.iloc[-1-n:,:])<0.025,2,0))
    ...
    rl_5p = rf.get_rolling_small(rd_20.iloc[-1000-n:,:],1000,950)
    rl_5p.dropna(how='all',inplace=True)
    #writer = cmn.to_file(rl_5p.sort_index(ascending=False, inplace=False),"31_5pDev",csv,str_trade_date)
    
    rl_12p = rf.get_rolling_small(rd_20.iloc[-1000-n:,:],1000,880)
    rl_12p.dropna(how='all',inplace=True)
    #writer = cmn.to_file(rl_12p.sort_index(ascending=False, inplace=False),"32_12pDev",csv,str_trade_date)
        
    rl_20p = rf.get_rolling_small(rd_20.iloc[-1000-n:,:],1000,800)
    rl_20p.dropna(how='all',inplace=True)
    ...
    array_cntr = np.where(rm_50.isnull() ,np.nan,
                          np.where(df_Stock.iloc[-1-n:,:] > rm_50,0,
                          np.where(rd_20.iloc[-1-n:,:] > rl_5p,3,
                          np.where(rd_20.iloc[-1-n:,:] > rl_12p,2,
                          np.where(rd_20.iloc[-1-n:,:] > rl_20p,1,0)))))
    indx = rm_50.index.values
